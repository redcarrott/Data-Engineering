...
def main():
    ...
    purchase_ingredient2 = purchase_ingredient \
        .select('raw_event', 'timestamp', 'Accept', 'Host', 
                'User-Agent', 'event_type', 'Content-Length', 'Content-Type', 
                from_json('attributes', ingredient_attr_schema()).alias('jsonData')) \
        .select('raw_event', 'Host','timestamp', 'event_type', 'jsonData.*')

    sink_purchase_ingredient = purchase_ingredient2 \
        .writeStream \
        .format("parquet") \
    ...
    join_restaurant2 = join_restaurant \
        .select('raw_event', 'timestamp', 'Accept', 'Host', 
                'User-Agent', 'event_type', 'Content-Length', 'Content-Type', 
                from_json('attributes', restaurant_attr_schema()).alias('jsonData')) \
        .select('raw_event', 'Host','timestamp', 'event_type', 'jsonData.*')
    
    sink_join_restaurant = join_restaurant2 \
        .writeStream \
        .format("parquet") \
    ...
    enter_contest2 = enter_contest \
        .select('raw_event', 'timestamp', 'Accept', 'Host', 
                'User-Agent', 'event_type', 'Content-Length', 'Content-Type', 
                from_json('attributes', contest_attr_schema()).alias('jsonData')) \
        .select('raw_event', 'Host','timestamp', 'event_type', 'jsonData.*')
    
    sink_enter_contest = enter_contest2 \
        .writeStream \
        .format("parquet") \
